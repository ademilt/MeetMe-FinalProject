...
class Adventures {
    ...
    func loadData(person: PersonalLocation, friend: FriendLocation, midpoint: MidpointLocation, completed: @escaping () -> ()) {
        ...
        db.collection("users").document(postingUserID).collection("adventures").document(person.documentID).collection("friend").document(friend.documentID).collection("midpoint").document(midpoint.documentID).collection("adventure").addSnapshotListener { querySnapshot, error in
