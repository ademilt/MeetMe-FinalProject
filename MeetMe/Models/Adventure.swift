...
class Adventure{
    ...
    convenience init(dictionary: [String: Any]) {
        ...
        let meetingLocation = dictionary["friendOriginalLocation"] as! String? ?? ""
...
//        if self.documentID == "" {
//            var ref: DocumentReference? = nil // Firestore will create a new ID for us
//            ref = db.collection("users").document(postingUserID).collection("adventures").document(person.documentID).collection("adventure").addDocument(data: dataToSave){ error in
//                guard error == nil else {
//                    print("😡 ERROR: Adding document \(error!.localizedDescription).")
//                    return completion(false)
//                }
...
//            }
//        } else { // else save to the existing document id with .setData
//            let ref = db.collection("users").document(postingUserID).collection("adventures").document(person.documentID).collection("adventure").document(self.documentID)
//            ref.setData(dataToSave) { error in
//                guard error == nil else {
//                    print("😡 ERROR: Updating document \(error!.localizedDescription).")
//                    return completion(false)
