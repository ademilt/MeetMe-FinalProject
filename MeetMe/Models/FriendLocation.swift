...
class FriendLocation: NSObject, MKAnnotation {
    ...
    func saveData(person: PersonalLocation, completion: @escaping (Bool) -> ()) {
        ...
        if self.documentID == "" {
            ...
            ref = db.collection("adventures").document(person.documentID).collection("friend").addDocument(data: dataToSave) { error in
                guard error == nil else {
                    print("😡 ERROR: Adding document \(error!.localizedDescription).")
                    return completion(false)
                }
                self.documentID = ref!.documentID
                print("😀 Added document \(self.documentID) to spot: \(person.documentID). It worked!")
                 
                    completion(true)
                
            }
        } else { // else save to the existing document id with .setData
            let ref = db.collection("adventures").document(person.documentID).collection("friend").document(self.documentID)
            ref.setData(dataToSave) { error in
                guard error == nil else {
                    print("😡 ERROR: Updating document \(error!.localizedDescription).")
                    return completion(false)
                }
                print("😀 Updated document \(self.documentID) in spot: \(person.documentID). It worked!")
